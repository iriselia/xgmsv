...
#define MESSAGE_SIZE(a) sizeof(a)/sizeof(char)
	...
	*/
	...
	{
		uint8* buffer = payload.data();

		uint64 payload_size = payload.len();
		...
		if (buffer[0] % 2)
		{
			decrypted_size = decompress_message(buffer, payload_size + payload.space(), workbuf + 1, decrypted_size);
		}
		else
		{
		...
		//payload.rpos++;
		payload.size += decrypted_size - payload_size;
		return true;
	}
	...
	{
		uint8* buffer = payload.data();
		const uint64 payload_size_with_0 = payload.len();

		if (payload.space() < payload_size_with_0)
		{
...
*/
		...
		payload.size += encrypted_size - payload_size_with_0;
