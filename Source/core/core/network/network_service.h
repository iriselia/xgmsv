...
namespace core
{
	...
	struct async_network_service
	{
		...
		template<typename lambda, typename lambda2, typename lambda3>
		task async_accept_loop(std::string ip, unsigned short port, lambda& accept_handler, lambda2& receive_handler, lambda3& send_handler, volatile bool& stop_signal);
	};


	template<typename lambda, typename lambda2, typename lambda3>
	task async_network_service::async_accept_loop(std::string ip, unsigned short port, lambda& accept_handler, lambda2& receive_handler, lambda3& send_handler, volatile bool& stop_signal)
	{
		try
		{
			while (!stop_signal)
